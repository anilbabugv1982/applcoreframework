...
public class ManageSetEnabledLookup extends GetDriverInstance {
	...
	public void setUp(String user, String passWord) throws Exception {
		try {
			...
			taskFlowsNavigation.navigateToAOLTaskFlows("Manage Set Enabled Lookups", isoDriver);
			CommonUtils.explicitWait(isoDriver.findElement(By.xpath("//h1[contains(text(),'Manage Set Enabled Lookups')]")),"Click", "",isoDriver);
			CommonUtils.hold(5);
		} catch (Exception e) {
			System.out.println("Exception in initializing objects in ManageSetEnabledLookup class:  " + e.getMessage());
			e.printStackTrace();
		}
	}

	@Test(description = "Create SetEnabled lookupType")
	public void testcase01() throws Exception {
		CommonUtils.hold(2);
		isoUtils.createLookupType("STD_" + id, "MEAN_" + id, "DESC_" + id, "set",isoDriver);
		Assert.assertTrue(isoUtils.verifyStatus("STD_" + id, "", "MEAN_" + id, "DESC_" + id,isoDriver), "DONE");
	}

	@Test(dependsOnMethods = "testcase01", description = "Create SetEnabled lookup code")
	public void testcase02() throws Exception {
		CommonUtils.hold(2);
		isoUtils.createLookupCode("STD_" + id, "STD_CODE_" + id, "MAEN_CODE_" + id, "DESC_CODE_" + id, "set",isoDriver);
		Assert.assertTrue(
				isoUtils.verifyStatus("STD_" + id, "STD_CODE_" + id, "MAEN_CODE_" + id, "DESC_CODE_" + id,isoDriver),
				"LookUpCode created");
	}

	@Test(dependsOnMethods = "testcase02", description = "Update SetEnabled lookup type")
	public void testcase03() throws Exception {
		isoUtils.updateLookupType("STD_" + id, "Update_m"+id, "update_d"+id,isoDriver);
		Assert.assertTrue(isoUtils.verifyStatus("STD_" + id, "", "Update_m"+id, "update_d"+id,isoDriver), "Updated");
	}

	@Test(dependsOnMethods = "testcase03", description = "Update SetEnabled lookup code")
	public void testcase04() throws Exception {
		isoUtils.updateLookupCode("STD_" + id, "STD_CODE_" + id, "CODE_M", "CODE_D", "set",isoDriver);
		Assert.assertTrue(isoUtils.verifyStatus("STD_" + id, "STD_CODE_" + id, "CODE_M", "CODE_D",isoDriver));

	}

	@Test(dependsOnMethods = "testcase04", description = "Delete SetEnabled lookup code")
	...
	@Test(dependsOnMethods = "testcase05", description = "Delete SetEnabled lookup type")
