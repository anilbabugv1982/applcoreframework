...
//	@FindBy(xpath="//h1[contains(text(),'Manage Common Lookups')]")
	...
	/*
	 * GetUpdatedPOValue(String ProfileCode,String UpdatedText) method will return updated lookup code value corresponding to profileoption from ProfileAdminstrationValues tf. 
	 * Lookup Type "FND_PAGE_COMPOSER_SOURCE_VIEW"
	 */
	public String GetUpdatedPOValue(String ProfileCode,String UpdatedText,WebDriver driver) {
		CommonUtils.hold(5);
		POCode.click();
		CommonUtils.hold(2);
		POCode.sendKeys(ProfileCode);
		CommonUtils.hold(3);
		POSearch.click();
		CommonUtils.hold(10);
		String PoUpdatedValue = driver.findElement(By.xpath("//option[text() = '"+UpdatedText+"']")).getText();
		
		return PoUpdatedValue;
	}
	
	/*
	 * GetUpdatedTreeStructureFieldValue(String UpdatedText) method will return updated "Status" field value from "Manage Tree Structures" tf that was updated from "Manage Standard Lookups" tf
	 * Lookup type -> "FND_TREE_STATUS_TYPE" <<-->> FieldName -> "Status"
	 * Lookup type -> "FND_TREE_CREATION_MODE_TYPE" <<-->> FieldName -> "Creation Mode"
	 */
	public String GetUpdatedTreeStructureFieldValue(String FieldName,String UpdatedText,WebDriver driver) {
		
		String UpdatedFieldValue = driver.findElement(By.xpath("//label[text() = '"+FieldName+"']/ancestor::tr[1]/descendant::option[text() = '"+UpdatedText+"']")).getText();
